@section('title', ('Editar Séries'))
<div class="font-mono container md:px-6 xl:px-28">
    <div class="mx-auto max-w-screen-sm text-center mb-4 lg:mb-8">
         <h2 class="mb-4 text-4xl tracking-tight font-extrabold text-gray-900 dark:text-white">Editar de Séries</h2>
     </div> 
     
             {{-- Filtro de pesquisa --}}
             <section class="section">
                <form action="{{{route('view.edit.serie')}}}" method="get">
                    @csrf
     ...
     <div class="grid gap-8 mb-8 mt-4 lg:mb-12 md:grid-cols-2">
        @foreach ($viewEseries as $serie)
         <div class="items-center  bg-gray-50 rounded-lg shadow sm:flex dark:bg-gray-800 dark:border-gray-700 hover:scale-105 hover:shadow-lg hover:shadow-slate-500">   
             <!-- Capa -->
                 <img class="h-[16rem] w-[14rem]" src="{{$serie->capa}}">
             ...
             <div class="p-4">
                ...
                <div class="space-x-14" >
                    <a href="{{route('edit.serie', $serie->id)}}"><i class="fa-solid fa-pen text-lg"></i></a>
                    ...
                    <p class="py-4"> Deseja excluir permanentemente {{$serie->titulo}}?</p>
                    <div class="modal-action">
                    
                        <label for="my-modal" class="btn">Ops, Me Enganei</label>

                    <form action="{{route('delete.serie', $serie->id)}}" method="post">
                        @csrf
                        @method('DELETE')
