        ...
        <div class="container grid mx-auto ">
            <div class="flex flex-col lg:flex-row justify-center items-strech mx-24">
                <div class="lg:w-6/12 flex justify-center items-center">
                    <div>
                        <h1 class="dark:text-white text-4x1 font-semibold text-black ">{{$viewanime->titulo}}</h1>

                        <p class="text-gray-800 dark:text-gray-600">
                        @php
                            $gene = trim($viewanime->genero, '[]""');
                            $gene = strtr($gene, '"', " ");
                        @endphp
                        {{$gene}}
                        </span>
                        ...
                        <p class="text-gray-800 dark:text-gray-600">Classificação: <span class="text-lg font-">
                            {{$viewanime->classificacao}}
                        </span></p>

                        <p class="mt-3 mb-4 font-light text-black dark:text-gray-600">{{$viewanime->sinopse}}</p>
                        ...
                        <ul class="flex space-x-4 sm:mt-0">
                            <li><!-- lancamento -->
                                ...
                                @php
                                    $lanca = new DateTime($viewanime->lancamento);
                                 @endphp
                                 {{$lanca->format('d/m/Y')}}
                                </span></p>
                            </li>
                            ...
                            <li>
                                <p class="text-gray-800 dark:text-gray-600">Duração de Episódio <span class="text-lg text-black">
                                    @php
                                        $dura = new DateTime ($viewanime->duracaoEpisodio);    
                                    @endphp
                                    {{$dura->format('H:i')}}
                                </span>
                            </li>
                ...
                <div class="lg:w-8/12 mt-4 md:mt-6 lg:mt-4 mx-8">
                    <div class="relative w-full h-full">
                        <img src="{{$viewanime->capa}}" class="w-[760px] h-[980px] relative" />
                    </div>
                </div>
            </div>
        </div>
